...
Spacify is open source and not notarized. The script clears the Gatekeeper download quarantine and applies a local ad-hoc signature; that is the "approve" step. It's a dozen readable lines: [`install.sh`](install.sh). Prefer doing it by hand? Grab the zip from [Releases](https://github.com/Aryagm/spacify/releases), drop the app in `/Applications`, and right-click it and choose Open the first time.

The release binary is universal (Apple silicon + Intel) and needs **macOS 14.2+**.

First run: click the earbuds icon in the menu bar, toggle an app, listen. macOS will ask for **System Audio Recording** permission the first time you spatialize something. That permission is the process tap.
...
- **Native AirPods head tracking.** One switch anchors the sound stage in front of you using the AirPods' own motion engine, and it toggles live without stopping the music. Spacify never touches the motion data itself.
...
What it deliberately does **not** do: EQ, gain, compression, limiting, reverb tweaks, stereo widening, crossfeed, or any custom DSP. What you hear is Apple's spatial render of the original stream and nothing else. A fixed "clean music" profile keeps the mixer honest: stereo stays an ambience bed, the mixer's default reverb is zeroed, and playback rate is locked at 1.0. The single exception is a sub-second fade when a route starts or stops so toggling doesn't click. Steady-state audio is never touched.
...
Selection changes and device switches rebuild the route through one debounced path. The old route fades out and is fully torn down before its replacement starts, because a second tap on already-tapped processes captures only silence. The new route holds a short settle window before fading in, so toggling sounds like a crossfade rather than a glitch.

### The real-time path

The render callback runs about 100 times per second on an audio thread, so the hot path is built to do as close to nothing as possible:

- **Zero-copy first.** When the tap delivers planar stereo float, the mixer's pull callback hands the tap buffers to the mixer by pointer, with no copy. When the output buffers are already planar, the mixer renders directly into them, with no scratch buffer.
- **vDSP for the rest.** Where layouts genuinely differ (interleaved vs. planar), conversion is a single SIMD `vDSP_ctoz`/`vDSP_ztoc` call. Matching layouts use `memcpy`. The worst case per cycle is two vDSP calls.
...
- The app is not notarized. The install script approves it locally (quarantine removal plus ad-hoc signature). Installing by hand means right-clicking the app and choosing Open the first time.
